...
func get_frame_time() -> float:
	var usec_delta: int = Time.get_ticks_usec() - last_usec
...
func os_time_per_frame():
	add_funcref_monitor(Callable(self, "get_frame_time"), [], "OS time / frame, s", 
...
func init_nodes():
	...
	margin_container.set("theme_override_constants/margin_top", SCREEN_MARGIN)
	margin_container.set("theme_override_constants/margin_bottom", SCREEN_MARGIN)
	margin_container.set("theme_override_constants/margin_left", SCREEN_MARGIN)
	margin_container.set("theme_override_constants/margin_right", SCREEN_MARGIN)
	var hbox_container = HBoxContainer.new()
	hbox_container.set("theme_override_constants/separation", SCREEN_MARGIN / 2)
...
func add_perf_monitor(param_key: int, 
	...
	var new_plot: MonitorPlotPanel = plot_scene_perf.instantiate()
...
func add_custom_monitor(obj: Object, param_name: String, 
					...
					data_len: int = DEFAULT_DATA_LEN, 
					size_px: Vector2 = DEFAULT_PLOT_SIZE):
	var new_plot: MonitorPlotPanel = plot_scene_custom.instantiate()
	plot_panels_node.add_child(new_plot)
	new_plot.setup_plot(obj, param_name, label, max_value, data_len, color, size_px, is_data_int, humanise)


func add_funcref_monitor(function_ref: Callable, function_params: Array = [], 
					label: String = "",  
					color: Color = DEFAULT_PLOT_COLOR, 
					humanise: bool = false, 
					is_data_int: bool = true, 
					...
					data_len: int = DEFAULT_DATA_LEN, 
					size_px: Vector2 = DEFAULT_PLOT_SIZE):
	var new_plot: MonitorPlotPanel = plot_scene_funcref.instantiate()
	plot_panels_node.add_child(new_plot)
