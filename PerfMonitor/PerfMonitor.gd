...
func _ready():
	...
	add_perf_monitor(Performance.TIME_PROCESS, "Time process, s", 
		Color(0.9, 0.9, 0.9, 0.6), false, 0.0001)
...
func add_perf_monitor(param_key: int, 
	...
	new_plot.setup_plot(param_key, label, max_value, color, humanise)
...
func add_custom_monitor(obj: Object, param_name: String, 
	...
	$Margin/HBox.add_child(new_plot)
	new_plot.setup_plot(obj, param_name, label, max_value, color, humanise)


func add_funcref_monitor(func_ref: FuncRef, func_params: Array = [], 
					label: String = "",  
					color: Color = DEFAULT_PLOT_COLOR, 
					humanise: bool = false, 
					max_value: float = DEFAULT_DATA_MAX):
	...
	$Margin/HBox.add_child(new_plot)
	new_plot.setup_plot(func_ref, func_params, label, max_value, color, humanise)
